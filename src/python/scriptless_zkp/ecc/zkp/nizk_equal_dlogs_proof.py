...
class NIZKEqualDiscreteLogsCommon:
    ...
    def generate_random_nonce(self) -> int:
        """
        Generates a random big integer in the range `[1, q]` inclusive, where `q` is the configured elliptic curve
        sub-group's order (i.e., `|<G>|` where `<G>` is the sub-group generated by the public base point `G`).
        :return: a random big integer in the range `[1, q]` inclusive, where `q` is the configured elliptic curve
                 sub-group's order.
        """
